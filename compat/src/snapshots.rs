...
pub fn load_manifest(repo_root: &Path) -> Result<SnapshotManifest> {
  ...
  let manifest: SnapshotManifest = serde_json::from_str(&raw)
    .with_context(|| format!("parse manifest json: {path:?}"))?;
  Ok(manifest)
}
...
impl SnapshotBundle {
  pub fn load(repo_root: &Path) -> Result<Self> {
    ...
    let openclaw_status: serde_json::Value = serde_json::from_str(&openclaw_status_raw)
      .context("parse openclaw_status.json")?;

    let openclaw_version_txt = read_text(base.join(&manifest.files.openclaw_version_txt))?;
    let runtime_tool_surface_md = read_text(base.join(&manifest.files.runtime_tool_surface_md))?;
  ...
  pub fn hashes(&self, repo_root: &Path) -> Result<SnapshotHashes> {
    ...
    add("openclawStatusJson", &self.manifest.files.openclaw_status_json)?;
    if let Some(h) = &self.manifest.files.openclaw_status_header {
      add("openclawStatusHeader", h)?;
    }
    add("openclawVersionTxt", &self.manifest.files.openclaw_version_txt)?;
    add("runtimeToolSurfaceMd", &self.manifest.files.runtime_tool_surface_md)?;
