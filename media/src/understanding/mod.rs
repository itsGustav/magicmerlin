...
pub use audio::{AudioFormat, AudioMetadata, AudioProvider};
pub use pdf::{PdfFallbackMode, PdfPageRange};
pub use routing::{ProviderCost, ProviderDecision};
pub use video::{KeyFrame, VideoPlan};
...
impl UnderstandingClient {
    ...
    pub async fn analyze(&self, request: AnalysisRequest) -> Result<AnalysisResult> {
        let decision =
            self.route_provider_by_cost(request.media_type, request.preferred_provider)?;
    ...
    async fn transcribe_audio(
        ...
        self.transcribe_audio_with_fallback(request).await
    }
    ...
    async fn analyze_video(
        ...
        self.analyze_video_with_plan(request, provider, VideoPlan::default())
            .await
    }
    ...
    async fn analyze_pdf(
        ...
        self.analyze_pdf_with_fallback(request, None, PdfFallbackMode::TextFirst)
            .await
    }
