...
Tools Invoke API covers an essential aspect of the MagicMerlin gateway. The gateway
...
Settings related to tools invoke api can be adjusted there.

```toml
[gateway]
# Tools Invoke API settings
enabled = true
```
...
```json
{
  "method": "gateway.tools-invoke-http-api",
  "params": {}
}
```
...
If you encounter issues with tools invoke api:
