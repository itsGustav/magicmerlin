...
OpenResponses API covers an essential aspect of the MagicMerlin gateway. The gateway
...
Settings related to openresponses api can be adjusted there.

```toml
[gateway]
# OpenResponses API settings
enabled = true
```
...
```json
{
  "method": "gateway.openresponses-http-api",
  "params": {}
}
```
...
If you encounter issues with openresponses api:
