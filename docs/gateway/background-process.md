...
Background Exec and Process Tool covers an essential aspect of the MagicMerlin gateway. The gateway
...
Settings related to background exec and process tool can be adjusted there.

```toml
[gateway]
# Background Exec and Process Tool settings
enabled = true
```
...
```json
{
  "method": "gateway.background-process",
  "params": {}
}
```
...
If you encounter issues with background exec and process tool:
