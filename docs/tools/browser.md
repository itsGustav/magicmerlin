...
Browser (OpenClaw-managed) is a built-in tool available to MagicMerlin agents. Tools extend the
...
{
  "name": "browser",
  "description": "Browser (OpenClaw-managed)",
  "parameters": {
    "type": "object",
    "properties": {
      "input": { "type": "string" }
...
that browser (openclaw-managed) capabilities are needed.
