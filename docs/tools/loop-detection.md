...
Tool-loop detection is a built-in tool available to MagicMerlin agents. Tools extend the
...
{
  "name": "loop-detection",
  "description": "Tool-loop detection",
  "parameters": {
    "type": "object",
    "properties": {
      "input": { "type": "string" }
...
that tool-loop detection capabilities are needed.
